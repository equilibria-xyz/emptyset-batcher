  ...
  context('funded', async () => {
    ...
    describe('#close', async () => {
      ...
      it('closes empty', async () => {
        const reserveDsuBalance = await proto.dsu.balanceOf(proto.reserve.address)
        const reserveUsdcBalance = await proto.usdc.balanceOf(proto.reserve.address)

        await expect(batcher.connect(proto.owner).close())
          .to.emit(batcher, 'Close')
          .withArgs(utils.parseEther('1000000'))

        expect(await proto.dsu.balanceOf(proto.owner.address)).to.equal(0)
        expect(await proto.usdc.balanceOf(proto.owner.address)).to.equal(0)

        expect(await proto.dsu.balanceOf(proto.user.address)).to.equal(0)
        expect(await proto.usdc.balanceOf(proto.user.address)).to.equal(1000000_000_000)

        expect(await proto.dsu.balanceOf(batcher.address)).to.equal(0)
        expect(await proto.usdc.balanceOf(batcher.address)).to.equal(0)

        expect(await proto.dsu.balanceOf(proto.reserve.address)).to.equal(
          reserveDsuBalance.add(utils.parseEther('1000000')),
        )
        expect(await proto.usdc.balanceOf(proto.reserve.address)).to.equal(reserveUsdcBalance)
      })

      it('closes not empty', async () => {
        const reserveDsuBalance = await proto.dsu.balanceOf(proto.reserve.address)
        const reserveUsdcBalance = await proto.usdc.balanceOf(proto.reserve.address)
      ...
      it('closes rounding', async () => {
        const expectedTotalDsu = utils.parseEther('1000000').add(utils.parseEther('0.000001')).sub(1)
        ...
        await expect(batcher.connect(proto.owner).close()).to.emit(batcher, 'Close').withArgs(expectedTotalDsu)
        ...
        expect(await proto.dsu.balanceOf(proto.reserve.address)).to.equal(reserveDsuBalance.add(expectedTotalDsu))
        expect(await proto.usdc.balanceOf(proto.reserve.address)).to.equal(reserveUsdcBalance)
      })

      it('closes not owner', async () => {
        await expect(batcher.connect(proto.user).close()).to.be.revertedWith(
          `UOwnableNotOwnerError("${proto.user.address}")`,
        )
      })
    })
  })
